...
<%
    try {
        ...
        if (request.getParameter("book") != null) {
            ...
            if (x > 0) {
                int amt=0;
                ResultSet rsa = stmt.executeQuery("select * from test where t_id='"+ tst +"'");
                while(rsa.next()){
                    amt = rsa.getInt(3);
                }
                ...
                if(rst.next()){
                        PreparedStatement pst1 = conn.prepareStatement("update transaction_info set fees=fees+"+ amt + "where p_id like'" + pid +"'");
                        pst1.executeUpdate();
                    }
                else{
                        ...
                        psb2.setInt(2, 0);
                        psb2.executeUpdate();
                        PreparedStatement psb3 = conn.prepareStatement("update transaction_info set fees=fees+" + amt + "where p_id like'" + pid +"'");
                        psb3.executeUpdate();
                    }
         ...
         if(request.getParameter("show") != null){
            ...
            Connection conn = DriverManager.getConnection("jdbc:derby://localhost:1527/hcare");
            Statement stmt = conn.createStatement(); 
            ResultSet rs = stmt.executeQuery("select t_id, t_name from test");
