...
<%
    try 
    {
        if(request.getParameter("b1") != null)
        {
            ...
            PreparedStatement pst = conn.prepareStatement("update EMPLOYEE_INFORMATION set E_ID=?,E_AGE=?,E_FIRSTNAME=?,E_MIDDLENAME=?,E_LASTNAME=?,E_EMAIL=?,DEPARTMENT=?,BLOCK_NO=?,HOUSE_NO=?,AREA=?,CITY=?,DISTRICT=?,STATE=?,ZIPCODE=?,E_PHNO=? where E_ID like ('"+e_id+"')");
